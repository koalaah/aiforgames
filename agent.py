...
class Agent(object):
    ...
    def __init__(self, world=None, scale=30.0, mass=1.0, mode='seek'):
        ...
        # spike07
        self.wanderAmount = 0
        self.separationAmount = 0
        self.alignmentAmount = 0
        self.cohesionAmount = 0
        self.MAX_STEERING_FORCE = 200
        self.neighbourRadius = 200
    ...
    def separation(self, group):
        ...
        for agent in group:
            if agent != self and agent.tagged:
                toAgent = self.pos - agent.pos
                steeringForce += toAgent.normalise() / toAgent.length()
    ...
    def flocking(self, delta):
        ...
        self.tagNeighbours(agents, self.neighbourRadius)

        steeringForce += self.wander(delta) * self.wanderAmount
        steeringForce += self.separation(agents) * self.separationAmount
        steeringForce += self.alignment(agents) * self.alignmentAmount
        steeringForce += self.cohesion(agents) * self.cohesionAmount

        steeringForce.truncate(self.MAX_STEERING_FORCE)
