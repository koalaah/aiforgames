...
class Path(object):
    ...
    def inc_current_pt(self):
        ...
        # move current to the next point
    ...
    def is_finished(self):
        ...
        # test for the end point in an open(non-loop) path
    ...
    def create_random_path(self, num_pts, minx, miny, maxx, maxy, looped=False):
        ...
        # Creates a new random path for us, to which we can pass world related parameters
        # (such a width and height)
        self.looped = looped
        self.clear()
        midX = (maxx + minx) / 2.0
        midY = (maxy + miny) / 2.0
    ...
    def set_pts(self, path_pts):
        ''' Replace our internal set of points with the container of points provided. '''
        self._pts = path_pts
        self._reset()

    def _reset(self):
        ''' Point to the first waypoint and set the limit count based on the number of points we've been given. '''
        self._cur_pt_idx = 0
    ...
    def get_pts(self):
        ''' Simple wrapper to return a reference to the internal list of points.'''
        return self._pts
